...
class Diffusion(nn.Module):
    ...
    def p_sample(self, x, t, t_index):
        device = self.device
        betas_t = extract(self.betas, t, x.shape, device)
        sqrt_one_minus_alphas_cumprod_t = extract(self.sqrt_one_minus_alphas_cumprod, t, x.shape, device)
        sqrt_recip_alphas_t = extract(self.sqrt_recip_alphas, t, x.shape, device)
    ...
    def p_losses(self, x_start, t, classes=None, noise=None, loss_type="huber", p_uncond=0.1):
        device = self.device
        noise = default(noise, torch.randn_like(x_start, device=device))
